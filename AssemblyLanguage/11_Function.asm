...
;ex) 두 값 중 더 큰 값을 반환하는 max함수를 만든다면?
; 근데 두 개의 값을 어떻게 넘겨받지? 반환은 어떻게 하지?
; ezx와 ebx 입력값을 ecx에 반환
MAX:
    cmp eax, ebx
    jg L1
    mov ecx, ebx
...
L2:
    ...
    ; [!] .data .bss 사용하면?
    ; 인자를 도대체 몇 개를 할당해야 하는가?
    
    ; 다른 메모리 구조가 필요하다
    ; - 꿈이 유효한 동안에는 그 꿈을 유지시켜야 함 (유효 범위의 개념이 있다)
    ; - 꿈이 끝나면 그 꿈을 부셔버려도 됨 (정리의 개념이 있다)
    ; - 꿈에서도 또 꿈응ㄹ 꿀 수 있다는 것을 고려해야 함 (유동적으로 유효 범위가 확장 가능)
